      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36 pb-20 relative z-20">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center min-h-[70vh]">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9 }}>
            <div className="inline-flex items-center gap-2 bg-green-50 border border-green-200 px-4 py-2 mb-8">
              <Leaf size={16} weight="fill" className="text-green-600" />
              <span className="text-green-700 text-[10px] font-bold uppercase tracking-[0.2em]">Eco-Friendly Options</span>
            </div>
            <div className="overflow-hidden">
              {['BUILDING', 'MATERIALS', 'SIMPLIFIED.'].map((line, i) => (
                <motion.div key={line} initial={{ y: '110%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.4 + i * 0.15, ease: [0.16, 1, 0.3, 1] }}>
                  <h1 className={`font-heading leading-[0.92] tracking-tight ${line === 'MATERIALS' ? 'text-green-600 italic font-bold' : 'text-gray-900 font-light'}`}
                    style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)' }}>{line}</h1>
                </motion.div>
              ))}
            </div>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}
              className="text-gray-500 text-sm leading-relaxed mt-6 max-w-md" style={{ fontFamily: 'var(--font-sans)' }}>
              {hero.badge}
            </motion.p>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1 }} className="flex flex-wrap gap-4 mt-8">
              <Link to="/contact" className="group inline-flex items-center gap-3 bg-green-600 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-green-700 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
                Request Quote <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link to="/services" className="group inline-flex items-center gap-3 border border-gray-300 text-gray-600 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:border-green-500 hover:text-green-600 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
                {hero.ctaSecondary}
              </Link>
            </motion.div>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.4 }} className="flex items-center gap-6 mt-10 pt-8 border-t border-gray-200">
              {[{ n: business.projectsCompleted, l: 'Customers' }, { n: business.yearsExperience, l: 'Years' }, { n: business.rating + '★', l: 'Rating' }].map((s, i) => (
                <div key={i}>
                  <div className="text-green-600 font-heading text-xl sm:text-2xl italic leading-none">{s.n}</div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            <div className="overflow-hidden aspect-[4/5]">
              <AnimatePresence mode="sync">
                <motion.img key={currentImg} src={heroImages[currentImg]} alt="Building supplies"
                  className="absolute inset-0 w-full h-full object-cover object-center"
                  initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
                  transition={{ duration: 1.5 }} loading="eager" />
              </AnimatePresence>
            </div>
            <div className="absolute -bottom-4 -left-4 bg-green-600 text-white p-6 shadow-xl z-10">
              <div className="text-center">
                <div className="font-heading text-3xl italic leading-none">{business.established}</div>
                <div className="text-green-200 text-[9px] uppercase tracking-[0.2em] mt-1" style={{ fontFamily: 'var(--font-sans)' }}>Since</div>
              </div>
            </div>
            <div className="absolute -top-3 -right-3 w-20 h-20 border-t-2 border-r-2 border-green-400/40" />
          </motion.div>
        </div>
      </div>
    </section>
...
function WhyTradeChecklist() {
  ...
  const items = [
    ...
    'Trade accounts with flexible terms',
    'All major building brands in stock',
    ...
    <section ref={ref} className="bg-gray-50 py-16 sm:py-20">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }}>
          <div className="w-12 h-[2px] bg-green-500 mb-6" />
          <h3 className="font-heading text-gray-900 text-xl italic mb-6">Why Trade With Us</h3>
          <div className="grid sm:grid-cols-2 gap-3">
            {items.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={inView ? { opacity: 1, x: 0 } : {}}
                transition={{ delay: i * 0.06 }} className="flex items-center gap-3 p-3">
                <CheckCircle size={18} weight="fill" className="text-green-500 shrink-0" />
                <span className="text-gray-500 text-sm" style={{ fontFamily: 'var(--font-sans)' }}>{item}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
      ...
      <div className="animate-marquee flex whitespace-nowrap">
        {repeated.map((item, i) => (
          <span key={i} className="flex items-center gap-6 sm:gap-8 mx-6 sm:mx-8">
            <span className="text-green-500 font-heading text-lg sm:text-2xl italic tracking-wider">{item}</span>
            <span className="text-green-500/30 text-sm">&diams;</span>
          </span>
        ))}
      </div>
    </section>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-green-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Services</p>
            ...
            <Link to="/services" className="group text-gray-400 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-green-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
              ...
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                ...
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`}
                  ...
                  <div className="absolute top-4 right-5 z-10">
                    <span className="text-green-500/15 font-heading text-6xl sm:text-7xl italic leading-none">{String(i + 1).padStart(2, '0')}</span>
                  </div>
                  <div className="absolute top-5 left-5 z-10 w-10 h-10 border border-green-500/30 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm group-hover:bg-green-500/20 group-hover:border-green-500/60 transition-all duration-500">
                    <IconComp size={18} weight="fill" className="text-green-500" />
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8 z-10">
                    ...
                    <div className="flex items-center gap-2 mt-3 text-green-500 group-hover:translate-x-1 transition-transform duration-300">
                      <span className="text-xs uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>Explore</span>
                      <ArrowRight size={14} />
                    </div>
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-green-500 to-green-300 z-10" />
                </Link>
              </motion.div>
            );
          }}
    ...
    <section ref={ref} className="bg-gray-50 py-24 sm:py-32">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-green-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Recent Work</p>
              <h2 className="font-heading text-gray-900 leading-[0.92] italic" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
                Featured <span className="text-green-500">Projects</span>
              </h2>
            </div>
            <Link to="/projects" className="group text-gray-400 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-green-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
          ...
          {projects.items.slice(0, 6).map((project, i) => (
            <motion.div key={project.slug} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              <div className={`overflow-hidden relative ${i === 0 ? 'aspect-square sm:aspect-auto sm:h-full' : 'aspect-[4/3]'}`}>
                ...
                <div className="absolute inset-0 border-2 border-transparent group-hover:border-green-500/30 transition-colors duration-500 z-10" />

                {/* Category badge */}
                <div className="absolute top-4 left-4 z-10">
                  <span className="bg-green-500/90 text-white text-[10px] uppercase tracking-[0.2em] px-3 py-1.5 font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>
                    {project.category}
                  </span>
                </div>
                ...
                {/* Number watermark */}
                <div className="absolute top-4 right-5 z-10">
                  <span className="text-green-500/10 font-heading text-5xl italic leading-none">{String(i + 1).padStart(2, '0')}</span>
                </div>
                ...
                {/* Bottom accent */}
                <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-green-500 z-10 scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />
              </div>
            </motion.div>
          ))}
        </div>
    ...
    <section ref={containerRef} className="bg-white py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 mb-12 sm:mb-16">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div>
            <div className="w-12 h-[2px] bg-green-500 mb-6" />
            <p className="text-green-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Selected Works</p>
          ...
          {projects.items.map((project, i) => (
            <div key={project.slug} className="group relative flex-shrink-0 w-[300px] sm:w-[360px] lg:w-[420px] overflow-hidden">
              <div className="aspect-[3/4] overflow-hidden relative">
                ...
                <div className="absolute inset-0 border-2 border-transparent group-hover:border-green-500/40 transition-colors duration-500 z-10" />
    ...
    <section ref={ref} className="bg-gray-50 py-20 sm:py-28">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14">
          <div className="w-12 h-[2px] bg-green-500 mb-6" />
          <h2 className="font-heading text-gray-900 leading-[0.95] italic" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
            Browse Our <span className="text-green-500">Range</span>
          </h2>
        </motion.div>
        <div className="flex gap-4 overflow-x-auto pb-4" style={{ scrollbarWidth: 'none' }}>
          {servicesPreview.map((cat, i) => (
            <motion.div key={cat.title} initial={{ opacity: 0, x: 20 }} animate={inView ? { opacity: 1, x: 0 } : {}}
              transition={{ delay: i * 0.08 }} className="flex-shrink-0 w-[260px] sm:w-[300px] group">
              <div className="aspect-[4/3] overflow-hidden relative">
                <img src={services?.items?.[i]?.image || ''} alt={cat.title} className="w-full h-full object-cover object-center group-hover:scale-110 transition-transform duration-700" loading="lazy" />
                <div className="absolute inset-0 bg-gradient-to-t from-gray-900/70 to-transparent" />
                <div className="absolute bottom-0 left-0 right-0 p-5">
                  <h3 className="text-white font-heading text-lg italic">{cat.title}</h3>
                </div>
              </div>
            </motion.div>
          ))}
  ...
  return (
    <section ref={ref} className="relative bg-white overflow-hidden">
      <div className="absolute inset-0">
        <div className="absolute top-1/2 left-1/4 -translate-y-1/2 w-48 h-48 bg-green-500/5 rounded-full blur-3xl" />
        <div className="absolute top-1/2 right-1/4 -translate-y-1/2 w-48 h-48 bg-green-500/5 rounded-full blur-3xl" />
      </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-16">
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              <div className="font-heading text-green-500 leading-none italic" style={{ fontSize: 'clamp(2.8rem, 6vw, 5rem)' }}>
              ...
              {i < stats.length - 1 && (
                <div className="hidden lg:block absolute right-0 top-1/2 -translate-y-1/2 h-16 w-[1px] bg-gradient-to-b from-transparent via-green-500/15 to-transparent" />
              )}
            </motion.div>
          ))}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            <div className="w-12 h-[2px] bg-green-500 mb-6" />
            <p className="text-green-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Story</p>
            <h2 className="font-heading text-gray-900 leading-[0.95] italic mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              The Nharadada<br /><span className="text-green-500">Story</span>
            </h2>
            {about.story.map((p, i) => (
            ...
            <div className="flex gap-10 sm:gap-16">
              <div>
                <div className="text-green-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.established}</div>
                <div className="text-gray-400 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Founded</div>
              </div>
              <div>
                <div className="text-green-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.projectsCompleted}</div>
                <div className="text-gray-400 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Completed</div>
              </div>
              <div>
                <div className="text-green-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.yearsExperience}</div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            ...
            <div className="absolute -top-3 -left-3 w-16 h-16 border-t-2 border-l-2 border-green-500/30" />
            <div className="absolute -bottom-3 -right-3 w-16 h-16 border-b-2 border-r-2 border-green-500/30" />
          </motion.div>
        </div>
      </div>
    </section>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            ...
            <div className="absolute -top-3 -left-3 w-20 h-20 border-t-2 border-l-2 border-green-500/40" />
            <div className="absolute -bottom-3 -right-3 w-20 h-20 border-b-2 border-r-2 border-green-500/40" />
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            transition={{ duration: 0.9, delay: 0.15 }}>
            <div className="w-12 h-[2px] bg-green-500 mb-6" />
            <p className="text-green-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>The Difference</p>
            <h2 className="font-heading text-gray-900 leading-[0.95] italic mb-12" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
              Why Choose <span className="text-green-500">Nharadada</span>
            </h2>
              ...
              {points.map((point, i) => (
                <motion.div key={point.title} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                  ...
                  <div className="shrink-0 mt-1">
                    <div className="w-8 h-8 border border-green-500/30 flex items-center justify-center bg-green-500/5">
    ...
    <section ref={ref} className="bg-white py-24 sm:py-32">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div className="w-12 h-[2px] bg-green-500 mx-auto mb-6" />
          <p className="text-green-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>What Drives Us</p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {about.values.map((value, i) => (
            <motion.div key={value.title} initial={{ opacity: 0, y: 25 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              className="relative p-6 sm:p-8 border border-gray-200 hover:border-green-500/40 transition-all duration-500 group">

              {/* Number watermark */}
              <span className="absolute top-4 right-4 text-green-500/8 font-heading text-6xl italic leading-none">
                {String(i + 1).padStart(2, '0')}
              </span>

              {/* Icon badge */}
              <div className="w-12 h-12 bg-green-500/10 flex items-center justify-center mb-5 group-hover:bg-green-500/20 transition-colors duration-500">
    ...
    <section ref={ref} className="relative bg-gray-50 py-24 sm:py-32 lg:py-40 overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center">
          <Quotes size={48} weight="fill" className="text-green-500/15 mx-auto mb-8" />
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex flex-col items-center gap-3">
                {t.avatar && <img src={t.avatar} alt={t.name} className="w-12 h-12 rounded-full object-cover object-center border-2 border-green-500/30" loading="lazy" />}
                ...
                <div className="flex items-center gap-0.5 mt-1">
                  {[...Array(t.rating)].map((_, i) => <Star key={i} size={12} weight="fill" className="text-green-500" />)}
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
          <div className="flex items-center justify-center gap-6 mt-12">
            <button onClick={prev} className="w-10 h-10 border border-gray-200 flex items-center justify-center text-gray-400 hover:text-green-500 hover:border-green-500/30 transition-colors" aria-label="Previous">
              <CaretLeft size={16} />
            </button>
            <div className="flex gap-2">
              {homeTestimonials.map((_, i) => (
                <button key={i} onClick={() => setActive(i)} className={`h-[2px] transition-all duration-500 ${i === active ? 'w-10 bg-green-500' : 'w-3 bg-gray-200'}`} aria-label={`Testimonial ${i + 1}`} />
              ))}
            </div>
            <button onClick={next} className="w-10 h-10 border border-gray-200 flex items-center justify-center text-gray-400 hover:text-green-500 hover:border-green-500/30 transition-colors" aria-label="Next">
              <CaretRight size={16} />
            </button>
          </div>
        </motion.div>
  ...
  return (
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-48 overflow-hidden">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <img src={siteData.hero.backgroundImages[0]?.url} alt="Nharadada Hardware" className="w-full h-[130%] object-cover object-center" loading="lazy" />
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 1 }}>
          ...
          <h2 className="font-heading text-white leading-[0.92] italic mb-8" style={{ fontSize: 'clamp(2.5rem, 8vw, 6rem)' }}>
            READY TO<br /><span className="text-green-500">GET STARTED?</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-green-500 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:shadow-xl transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
          ...
          <div className="mt-16 pt-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-center gap-6 text-xs text-white/25 uppercase tracking-[0.15em]" style={{ fontFamily: 'var(--font-sans)' }}>
            <a href={`tel:${business.phoneRaw}`} className="flex items-center gap-2 hover:text-green-500 transition-colors"><Phone size={14} /> {business.phone}</a>
