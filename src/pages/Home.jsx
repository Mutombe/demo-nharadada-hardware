...
import {
  ...
  CaretLeft,
  CaretRight,
  Briefcase, Buildings, Heart, Leaf, Lightbulb,
  CheckCircle,
...
/* ================================================================
   UTILITY -- Icon map
   ================================================================ */
const iconMap = { Briefcase, Buildings, Heart, Leaf, Lightbulb, Star };
      ...
      {[...Array(18)].map((_, i) => (
        <div
          ...
          style={{
            ...
            background: `radial-gradient(circle, rgba(34,197,94,${Math.random() * 0.5 + 0.2}) 0%, transparent 70%)`,
...
function HeroSection() {
  ...
  return (
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-neutral-950">
      {/* Crossfade carousel with parallax */}
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-neutral-950/70 via-transparent to-neutral-950/30 z-[1]" />
      </motion.div>
      ...
      <div className="absolute right-5 sm:right-8 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-3">
        {heroImages.map((_, i) => (
          <button key={i} onClick={() => setCurrentSlide(i)} className={`w-[3px] transition-all duration-700 ${i === currentSlide ? 'h-10 bg-amber-500' : 'h-4 bg-white/20 hover:bg-white/40'}`} aria-label={`Slide ${i + 1}`} />
        ))}
      </div>
      ...
      {/* Left accent line */}
      <div className="absolute top-[15%] left-0 w-[2px] h-32 sm:h-48 bg-gradient-to-b from-transparent via-amber-500 to-transparent z-20" />
      ...
      <motion.div className="relative z-20 flex flex-col justify-center h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36" style={{ y: textY, opacity }}>
        <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 1, delay: 0.3, ease: 'easeOut' }} className="w-16 h-[2px] bg-gradient-to-r from-amber-500 to-amber-400/50 mb-6 origin-left" />
        ...
        <div className="overflow-hidden">
          {['YOUR BUILDING', 'PARTNER', 'SINCE DAY ONE.'].map((line, i) => (
            <motion.div key={line} initial={{ y: '110%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.5 + i * 0.15, ease: [0.16, 1, 0.3, 1] }}>
              <h1
                className={`font-heading leading-[0.92] tracking-tight ${line === 'PARTNER' ? 'bg-gradient-to-r from-amber-500 via-amber-400 to-amber-500 bg-clip-text text-transparent italic' : 'text-white'}`}
                style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)', fontWeight: line === 'PARTNER' ? 700 : 300 }}
              >
                {line}
              </h1>
            </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 1.2 }} className="flex items-center gap-3 mt-8">
          ...
          <p className="text-white/30 text-xs sm:text-sm uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>
            {siteData.stats[0].number} {siteData.stats[0].label} &middot; {siteData.stats[2].number} {siteData.stats[2].label}
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.4 }} className="flex flex-wrap gap-4 mt-10">
          <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-amber-500 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:bg-amber-400 hover:shadow-xl hover:shadow-amber-500/20" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaPrimary}
            <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
          </Link>
          <Link to="/services" className="group inline-flex items-center gap-3 border border-white/20 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:border-amber-500/50 hover:text-amber-400 hover:bg-white/5" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaSecondary}
          </Link>
        </motion.div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }} className="absolute bottom-8 sm:bottom-12 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-2">
        ...
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2, ease: 'easeInOut' }}>
          <ArrowDown size={14} className="text-amber-500/40" />
        </motion.div>
      </motion.div>
      ...
      <div className="hidden lg:flex absolute right-8 bottom-12 z-20">
        <span className="text-white/10 text-[10px] uppercase tracking-[0.4em]" style={{ writingMode: 'vertical-rl', fontFamily: 'var(--font-sans)' }}>
          Nharadada Hardware &mdash; 523V+895
        </span>
      </div>
    </section>
  );
...
function MarqueeTicker() {
  const items = ['BUILDING MATERIALS', 'PLUMBING', 'ELECTRICAL', 'PAINT & FINISHES', 'TOOLS & HARDWARE', 'ROOFING'];
  const repeated = [...items, ...items, ...items, ...items];

  return (
    <section className="bg-neutral-950 border-y border-amber-500/10 py-5 sm:py-6 overflow-hidden">
      <div className="animate-marquee flex whitespace-nowrap">
        {repeated.map((item, i) => (
          <span key={i} className="flex items-center gap-6 sm:gap-8 mx-6 sm:mx-8">
            <span className="text-amber-500/80 font-heading text-lg sm:text-2xl italic tracking-wider">{item}</span>
            <span className="text-amber-500/20 text-sm">&diams;</span>
          </span>
        ))}
      </div>
    </section>
...
/* ================================================================
   3. SERVICES GRID -- Cards with image backgrounds
   ================================================================ */
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-amber-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Services</p>
              <h2 className="font-heading text-white leading-[0.92] italic" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
                What We <span className="text-amber-500">Deliver</span>
              </h2>
            </div>
            <Link to="/services" className="group text-white/30 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-amber-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
              ...
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.08 * i }} className={i === 0 ? 'sm:col-span-2 lg:col-span-2' : ''}>
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`} className={`group relative block overflow-hidden ${i === 0 ? 'aspect-[16/9] sm:aspect-[2/1]' : 'aspect-[3/4]'}`}>
                  ...
                  <div className="absolute top-4 right-5 z-10">
                    <span className="text-amber-500/15 font-heading text-6xl sm:text-7xl italic leading-none">{String(i + 1).padStart(2, '0')}</span>
                  </div>

                  {/* Icon badge */}
                  <div className="absolute top-5 left-5 z-10 w-10 h-10 border border-amber-500/30 flex items-center justify-center bg-neutral-950/60 backdrop-blur-sm group-hover:bg-amber-500/20 group-hover:border-amber-500/60 transition-all duration-500">
                    <IconComp size={18} weight="light" className="text-amber-500" />
                  </div>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8 z-10">
                    ...
                    <div className="flex items-center gap-2 mt-3 text-amber-500 group-hover:translate-x-1 transition-transform duration-300">
  ...
  return (
    <section ref={ref} className="relative bg-neutral-950 overflow-hidden">
      <div className="absolute inset-0">
        <div className="absolute top-1/2 left-1/4 -translate-y-1/2 w-48 h-48 bg-amber-500/5 rounded-full blur-3xl" />
        <div className="absolute top-1/2 right-1/4 -translate-y-1/2 w-48 h-48 bg-amber-500/5 rounded-full blur-3xl" />
      </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-16">
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: i * 0.12 }} className="text-center relative">
              <div className="font-heading text-amber-500 leading-none italic" style={{ fontSize: 'clamp(2.8rem, 6vw, 5rem)', textShadow: '0 0 40px rgba(34,197,94,0.15)' }}>
              ...
              {i < stats.length - 1 && (
                <div className="hidden lg:block absolute right-0 top-1/2 -translate-y-1/2 h-16 w-[1px] bg-gradient-to-b from-transparent via-amber-500/15 to-transparent" />
              )}
            </motion.div>
          ))}
        </div>
    ...
    <section ref={ref} className="bg-neutral-950 py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, ease: 'easeOut' }}>
            <div className="w-12 h-[2px] bg-amber-500 mb-6" />
            <p className="text-amber-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Story</p>
            <h2 className="font-heading text-white leading-[0.95] italic mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              About<br /><span className="text-amber-500">{business.name}</span>
            </h2>
            ...
            <div className="flex gap-10 sm:gap-16">
              <div>
                <div className="text-amber-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.established}</div>
                <div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Founded</div>
              </div>
              <div>
                <div className="text-amber-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.projectsCompleted}</div>
                <div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Projects</div>
              </div>
              <div>
                <div className="text-amber-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.employees}</div>
                <div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Team</div>
              </div>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2, ease: 'easeOut' }} className="relative">
            <div className="relative">
              <div className="overflow-hidden">
                <img src={hero.backgroundImages?.[0]?.url || siteData.hero.backgroundImages[0].url} alt={`${business.name} operations`} className="w-full aspect-[4/5] object-cover object-center" loading="lazy" />
              </div>
              <div className="absolute -bottom-8 -left-6 sm:-left-10 w-[45%] overflow-hidden border-4 border-neutral-950 shadow-2xl">
                <img src={siteData.hero.backgroundImages[1]?.url} alt={`${business.name} team`} className="w-full aspect-square object-cover object-center" loading="lazy" />
              </div>
              <div className="absolute -top-4 -right-4 sm:-right-6 bg-amber-500 text-white p-5 sm:p-7 shadow-2xl">
...
function WhyChooseUs() {
  ...
  const points = [
    { title: 'Massive Stock Range', desc: 'Over 5,000 products in stock. If we do not have it, we will source it within 48 hours.' },
    { title: 'Expert Staff', desc: 'Our team knows building materials inside out. Free advice on specifications, quantities, and alternatives.' },
    { title: 'Competitive Pricing', desc: 'Trade and bulk discounts that keep your project on budget without compromising quality.' },
    { title: 'Delivery to Site', desc: 'We deliver to your doorstep or construction site. On time, every time, anywhere in Harare.' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }} className="relative">
            <img src={siteData.hero.backgroundImages[1]?.url} alt="Our team at work" className="w-full aspect-[4/5] object-cover object-center" loading="lazy" />
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.15 }}>
            <div className="w-12 h-[2px] bg-amber-500 mb-6" />
            <p className="text-amber-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>The Difference</p>
              ...
              {points.map((point, i) => (
                <motion.div key={point.title} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.3 + i * 0.1 }} className="flex gap-5">
                  <div className="shrink-0 mt-1">
                    <div className="w-8 h-8 border border-amber-500/30 flex items-center justify-center bg-amber-500/5">
...
/* ================================================================
   7. PORTFOLIO GALLERY -- Horizontal scrolling
   ================================================================ */
function PortfolioGallery() {
  ...
  return (
    <section ref={containerRef} className="bg-neutral-950 py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 mb-12 sm:mb-16">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
          <div>
            <div className="w-12 h-[2px] bg-amber-500 mb-6" />
            <p className="text-amber-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Selected Works</p>
          ...
          <Link to="/projects" className="group text-white/30 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-amber-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 1, delay: 0.2 }}>
        <div ref={scrollRef} className="flex gap-4 sm:gap-5 overflow-x-auto px-5 sm:px-8 lg:px-12 pb-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', WebkitOverflowScrolling: 'touch' }}>
          {projects.items.map((project, i) => (
            <div key={project.slug} className="group relative flex-shrink-0 w-[300px] sm:w-[360px] lg:w-[420px] overflow-hidden">
              <div className="aspect-[3/4] overflow-hidden relative">
                ...
                <div className="absolute inset-0 bg-neutral-950/0 group-hover:bg-neutral-950/40 transition-colors duration-700" />
                <div className="absolute inset-0 border-2 border-transparent group-hover:border-amber-500/40 transition-colors duration-500 z-10" />
                <div className="absolute top-5 left-5 z-10">
                  <span className="bg-amber-500/90 text-white text-[10px] uppercase tracking-[0.2em] px-3 py-1.5 font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>
                    {project.category}
                  </span>
                </div>
              </div>
...
function ProcessSection() {
  ...
  const steps = [
    { num: '01', title: 'Get In Touch', desc: 'Contact us via phone, WhatsApp, or our online form. Tell us what you need.' },
    { num: '02', title: 'Free Consultation', desc: 'We assess your requirements and provide a detailed, no-obligation quote.' },
    { num: '03', title: 'We Deliver', desc: 'Our team executes with precision, keeping you updated every step of the way.' },
    { num: '04', title: 'Your Satisfaction', desc: 'We follow up to ensure everything meets your expectations. Your satisfaction is our guarantee.' },
  ];
    ...
    <section ref={ref} className="bg-neutral-900 py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center mb-16 sm:mb-20">
          ...
          <p className="text-amber-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>How It Works</p>
          <h2 className="font-heading text-white leading-[0.95] italic" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
            Simple <span className="text-amber-500">Process</span>
          </h2>
        </motion.div>

        <div className="relative grid md:grid-cols-4 gap-8 sm:gap-10">
          {/* Connecting line */}
          <div className="hidden md:block absolute top-12 left-[12.5%] right-[12.5%] h-[1px] bg-gradient-to-r from-transparent via-amber-500/20 to-transparent z-0" />

          {steps.map((step, i) => (
            <motion.div key={step.num} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: i * 0.15 }} className="relative z-10 text-center">
              <div className="w-16 h-16 mx-auto mb-6 border border-amber-500/30 flex items-center justify-center bg-neutral-950 group-hover:bg-amber-500/10 transition-colors">
                <span className="text-amber-500 font-heading text-2xl italic">{step.num}</span>
              </div>
              <h3 className="font-heading text-white text-lg italic tracking-wide mb-2">{step.title}</h3>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex flex-col items-center gap-3">
                {t.avatar && <img src={t.avatar} alt={t.name} className="w-12 h-12 rounded-full object-cover object-center border-2 border-amber-500/30" loading="lazy" />}
                ...
                <div className="flex items-center gap-0.5 mt-1">
                  {[...Array(t.rating)].map((_, i) => <Star key={i} size={12} weight="fill" className="text-amber-500" />)}
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
          ...
          <div className="flex items-center justify-center gap-6 mt-12">
            <button onClick={prev} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-amber-500 hover:border-amber-500/30 transition-colors duration-300" aria-label="Previous">
              <CaretLeft size={16} />
            </button>
            <div className="flex gap-2">
              {homeTestimonials.map((_, i) => (
                <button key={i} onClick={() => setActive(i)} className={`h-[2px] transition-all duration-500 ${i === active ? 'w-10 bg-amber-500' : 'w-3 bg-white/10 hover:bg-white/25'}`} aria-label={`Testimonial ${i + 1}`} />
              ))}
            </div>
            <button onClick={next} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-amber-500 hover:border-amber-500/30 transition-colors duration-300" aria-label="Next">
              <CaretRight size={16} />
            </button>
          </div>
        </motion.div>
...
function CTASection() {
  ...
  return (
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-48 overflow-hidden">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <img src={homeCta.backgroundImage} alt={business.name} className="w-full h-[130%] object-cover object-center" loading="lazy" />
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 1 }}>
          ...
          <h2 className="font-heading text-white leading-[0.92] italic mb-8" style={{ fontSize: 'clamp(2.5rem, 8vw, 6rem)' }}>
            BUILD YOUR<br /><span className="text-amber-500">VISION</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-amber-500 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:bg-amber-400 hover:shadow-xl hover:shadow-amber-500/25" style={{ fontFamily: 'var(--font-sans)' }}>
              {homeCta.ctaPrimary}
              <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
            </Link>
            <a href={`https://wa.me/${business.whatsappNumber}?text=${encodeURIComponent(homeCta.whatsappText)}`} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-3 border border-green-500/40 text-green-400 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:bg-green-500/10 hover:border-green-500/60" style={{ fontFamily: 'var(--font-sans)' }}>
          ...
          <div className="mt-16 pt-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-center gap-6 text-xs text-white/25 uppercase tracking-[0.15em]" style={{ fontFamily: 'var(--font-sans)' }}>
            <a href={`tel:${business.phoneRaw}`} className="flex items-center gap-2 hover:text-amber-500 transition-colors">
              <Phone size={14} /> {business.phone}
            </a>
            <span className="hidden sm:inline text-white/10">|</span>
            <a href={`mailto:${business.email}`} className="hover:text-amber-500 transition-colors">{business.email}</a>
            <span className="hidden sm:inline text-white/10">|</span>
